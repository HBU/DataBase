...
void CMainFrame::OnMenuLRC()  // for get histogram
{
	...
	**************************************/
	...
	AfxMessageBox("µÚÒ»²½£ºÔØÈëÊý¾Ý¼¯¡£Ê±¼ä½Ï³¤£¬ÇëÄÍÐÄµÈ´ý¡£");
	...
	else
	{
		...
		sprintf_s(temp, " Êý¾Ý¼¯ÔØÈëÍê±Ï , ¹²  %i Ìõ¼ÇÂ¼", iSizeofKB);
		AfxMessageBox(temp);
	}
//1. partion the domain of the dataset
//  division the biggest scr into 12 = 4*3 partition or more ----
//            select * From each partition,
	AfxMessageBox("µÚ¶þ²½£º·Ö¸îÊý¾Ý¼¯¡£Ê±¼ä½Ï³¤£¬ÇëÄÍÐÄµÈ´ý¡£");
    ...
    scr.v=1;
   // get domain rect
	//AfxMessageBox("ZLRECT¿Õ¼äÉêÇëÍê±Ï");
	for(i=1; i<=n; i++ )
	{
		scr.a[i] = pApp->dMin[i]; 
		scr.b[i] = pApp->dMax[i];  
	...
	//AfxMessageBox("scr³õÊ¼»¯Íê±Ï");
   // get tuples in domain, but we use the old struct, »ñÈ¡ÇøÓòÄÚµÄÔª×é£¬µ«ÎÒÃÇÊ¹ÓÃ¾ÉµÄ½á¹¹Ìå    
	pApp->clsNum =0;	
	for(i=1; i<=m; i++)
	{
		for(j= 1; j <=n; j++)
	...
	AfxMessageBox("ZLRECT T ³õÊ¼»¯Íê±Ï");
	int deepNum=1;
	if(n==2) //only for 2_dimensional ¶Ô¶þÎ¬Êý¾Ý·Ö¸î
		iPartNum = pApp->zlPartion(scr, n, T, m, iPartNum);
	else	 //for 3,4, 25,50,and 104 dimensions ¶ÔÈýÎ¬¼°¸ü¸ßÎ¬Êý¾Ý·Ö¸î
		iPartNum = pApp->zlPartion3D(&scr, n, T, m, iPartNum,deepNum);

//int s1=0,s2=0,f1=0,f2=0,f3=0;

//2. get the histogram
	AfxMessageBox("µÚÈý²½£º»ñÈ¡Ö±·½Í¼¡£Ê±¼ä½Ï³¤£¬ÇëÄÍÐÄµÈ´ý¡£");
	int Num=iPartNum;
	int ss=0;
    ZLRECT * cell ; 
...
//3. Select all queires from workload table ´Óworklord±íÔØÈëÈ«²¿²éÑ¯
AfxMessageBox("µÚ£¿²½£ºÔØÈë²éÑ¯²âÊÔ¼¯¡£Ê±¼ä½Ï³¤£¬ÇëÄÍÐÄµÈ´ý¡£");
////^^^^^^^
