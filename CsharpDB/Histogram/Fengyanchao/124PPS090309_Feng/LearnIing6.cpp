  ...
  //  ZLRECT box[65];
   ...
   int Numcell =0;   //this is p
    ...
    if(scr.b[1] - scr.a[1] < scr.b[2] - scr.a[2] )
	{
        ...
        if( (scr.b[2] - scr.a[2])/(scr.b[1] - scr.a[1]) >=64 )
		{
           l3 =1;
		   l4 = 64;             
		}
...
//	CString cstrMarc;
 ...
 /*    for(i=1; i<=12; i++)
	{
		memset(&box[i], 0, sizeof(ZLRECT));
		memcpy(&box[i], Rect, sizeof(ZLRECT));
	}//the shinked part*/
...
/*	for(i=1; i<=m; i++)
	...
	if(dim[1] != 1)
	{
		dMaxInterval2 = scr->b[1] - scr->a[1];
		dim[2] =1;
	}
	else
	{
		dMaxInterval2 = scr->b[2] - scr->a[2];
		dim[2] =2;
	}
	...
	{  
		if(i != dim[1] )
		{
			if(dMaxInterval2 < scr->b[i] - scr->a[i] )
			{
				dMaxInterval2 = scr->b[i] - scr->a[i];
				dim[2] =i;
			}
		}
	}
...
/////////////////////////////////////////////////////////////	
        ...
        minInterval = scr->b[1] - scr->a[1];

      for (i=1;i<=n;i++)
	  {
		  if ( minInterval >(scr->b[i] - scr->a[i]))
		  {
              minInterval=scr->b[i] - scr->a[i];
		  }

	  }
         minInterval=INT(minInterval/10)*10;
    double ratio1=(scr->b[dim[1]] - scr->a[dim[1]])/minInterval;
    double ratio2=(scr->b[dim[2]] - scr->a[dim[2]])/minInterval;
  if(PartNum==1)
  {
	  d2=160;
	  d3=3;
  ...
  else
  {
	  ...
	  else
	  {
		  ...
		  {
			d2=INT((scr->b[dim[1]] - scr->a[dim[1]])/10)*10/minInterval;
		  }
	 ...
	 { 
		 if(d2>=30)
		 {
		  ...
		  else if((INT(ratio2)==1)||((d2>=20)&&(d2<30)))
		  {
		      d3=2;
		  }
		  else
		  {
               d3=INT((scr->b[dim[2]] - scr->a[dim[2]])/minInterval);
		  }

	 }
	 }
	   a[dim[1]] = scr->a[dim[1]]; 
	   d[dim[1]] =(scr->b[dim[1]] - scr->a[dim[1]])/d2;
       a[dim[2]] = scr->a[dim[2]]; 
	   d[dim[2]] = (scr->b[dim[2]] - scr->a[dim[2]])/d3;

       a[dim[3]] = scr->a[dim[3]];
	   
	   if(PartNum!=1)
	   {
	   if((INT(ratio2)==1)&&(INT(ratio1)==1))
	   {
          d4=2;
       }
	   else if(d2*d3>30)
   	   ...
   	   }else
	   {
        ...
        d[dim[3]] = (scr->b[dim[3]] - scr->a[dim[3]])/d4;
       Numcell=d2*d3*d4;
	...
	for(k =1; k<=d2; k++)
	{
		for(j =1; j<=d3; j++)
		{
			for(i =1; i<=d4; i++)
			{
				...
				for(h=1; h<=COL_NUM; h++ ) //the Number of dimensions: NO1, NO2, NO3
				{					
					if(dim[h]== dim[1])
					{
						 part[i+d4*(j-1)+d4*d3*(k-1)].a[dim[1]] =  a[dim[1]] + (k-1) * d[dim[1]];
                        if(i!=d2)
						{    
						   part[i+d4*(j-1)+d4*d3*(k-1)].b[dim[1]] =  a[dim[1]] + k * d[dim[1]];
						}
						else
						{
                           part[i+d4*(j-1)+d4*d3*(k-1)].b[dim[1]]=scr->b[dim[1]];
						}
					}
					else if (dim[h]== dim[2]) 
					{
						 part[i+d4*(j-1)+d4*d3*(k-1)].a[dim[2]] =  a[dim[2]] + (j-1) * d[dim[2]];
						if(j!=d3)
						{
						 part[i+d4*(j-1)+d4*d3*(k-1)].b[dim[2]] =  a[dim[2]] + j * d[dim[2]];
						}
						else
						{
						 part[i+d4*(j-1)+d4*d3*(k-1)].b[dim[2]]= scr->b[dim[2]];
						}
					}
					else if (dim[h]== dim[3])
					{
						    part[i+d4*(j-1)+d4*d3*(k-1)].a[dim[3]] =  a[dim[3]] + (i-1) * d[dim[3]];
						if(k!=d4)
						{
						    part[i+d4*(j-1)+d4*d3*(k-1)].b[dim[3]] =  a[dim[3]] + i * d[dim[3]];
						}
						else
						{
							part[i+d4*(j-1)+d4*d3*(k-1)].b[dim[3]] =scr->b[dim[3]];
						}
					}
				}
			}
   ...
   for(i= 1; i<=Numcell; i++)
	{
	   part[i].v =1;
		...
		for(k=1;k<=n;k++)
		{
           part[i].v*=(part[i].b[k]-part[i].a[k]);
		}
	   ...
	   for(j= 1; j <=m ; j++) // m is the number of T[i]'s
		{
			...
			{				 //YES! do it here
             for(i= 1; i<= Numcell ; i++)
			 {
				...
				if(bInterable && T[j].suf1st<=0) //if intersectable
				{
					idx[i]++; 
                    T[j].suf1st =1;
				   s++;
         ...
         for (i=1;i<=Numcell;i++)
        {
		 ...
		 if (partmin==0)
		 {
          ...
          if (((partmax - partmin)/(partmax+1)<1/15)||(SumNum<=100))  
		  {
			  bpartion=false; 
			   ...
			   for (j=i+1;j<=Numcell;j++)
			   {
				   for (k=1;k<=n;k++)
				   {
					   for (k1=1;k1<=n;k1++)
					   {
						   if (k==k1)
						   {
							   if ((part[i].a[k1]==part[j].b[k1]) || (part[i].b[k1]==part[j].a[k1]))
							   {
								   bmerge=true;
							   }
							   else
						   ...
						   {
                            if(( part[i].a[k1]==part[j].a[k1])&&(part[i].b[k1]==part[j].b[k1]))
							{
								bmerge=true;
							}
							else
					   ...
					   if(bmerge)
					   {
						   if((idx[i]!=0)&&(idx[i]!=0))
						   {
                         	   densityi=idx[i]/part[i].v;
							   densityj=idx[j]/part[j].v;
                             
							  mm1=(fabs(densityj-densityi)/densityi);
							  mm2=(fabs(densityi-densityj)/densityj);
                         	...
                         	if((mm1<=mm0 )&&(mm2<=mm0))
							{
								...
								for(j1=1;j1<=n;j1++)
								{
                                    ...
                                    part[i].v*=part[i].b[j1]-part[i].a[j1];
